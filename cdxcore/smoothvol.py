"""
Non-homogeneous strike fitting via the SANOS formulation.
...
def focus_line(left : float, 
    ...
    x     = np.linspace( - 1.,  +1., N )
    x     = x*(1.-concentration)+concentration*x*( np.power(np.abs(x), power-1 ) if power>1 else 1.)
    ...
    * $C^i_J := c^i_j/F_j/\mathrm{DF}_j$ are pure market prices.
    ...
    def __init__(self,  label    : str|int, 
        ...
        if self.iv_bids is None:
            self.iv_bids = bs.implied( self.strikes, self.bids, sqrtT=self.sqrtT, ret_only_vols=True, on_exceed_bounds="quiet" ).astype(dtype,copy=False)
        else:
        ...
        if self.iv_asks is None:
            self.iv_asks = bs.implied( self.strikes, self.asks, sqrtT=self.sqrtT, ret_only_vols=True, on_exceed_bounds="quiet"  ).astype(dtype,copy=False)
        else:
...
class ExpiryFit( ExpiryData ):
    ...
    def __call__( self, strikes : np.ndarray|float, *, and_vols : bool = False ) -> np.ndarray|float|tuple[np.ndarray,np.ndarray]|tuple[float,float]:
        ...
        Parameters
        ==========
            strikes : np.ndararray | float
                Strikes

            and_vols : bool
                Whether to also compute implied vols

        Returns
        =======
            prices : np.ndarray | float
                Prices if ``and_vols`` is ``False``.

            prices, vols : tuple[np.ndarray, np.ndarray] | tuple[float|float]
                Prices and vols if ``and_vols`` is ``True``.
        """
        ...
        calls = ( bs.price( strikes[:,None] / self.xstrikes[None,:], self.sqrtT*self.xvols[None,:] ) * self.xstrikes[None,:] ) @ self.xdensity
        assert calls.shape == strikes.shape
        if not and_vols:
            return calls

        vols = bs.implied( k=strikes, prices=calls, sqrtT=self.sqrtT, on_exceed_bounds="quiet", ret_only_vols=True ).astype(self.dtype,copy=False)
        return calls, vols
    ...
    def _priceT( self, strikes : np.ndarray|float, *, sqrtT : float|None = None ) -> np.ndarray|float|tuple[np.ndarray,np.ndarray]|tuple[float,float]:
        ...
        """
        if isinstance(strikes, (float, int)):
            r = self( np.array( [strikes], dtype=self.dtype ), and_vols=False )
            return r
...
class SmoothCallSurfaceConfig:
    """ Configuration for ``SmoothCallSurface`` implementing [SANOS](https://arxiv.org/abs/2601.11209) """
    dtype          : type = None       # dtype. If not specified, infer from first strike array.
    ...
    bid_ask_mode   : str = "ignore"    # how to handle bid/ask: ``penalty``, 'ignore', 'constraint'; see [SANOS paper](https://arxiv.org/abs/2601.11209)
    spread_weighted: bool = True       # weight objective by spread

    # optimizer settings
    solver         : str = "CLARABEL"  # `cvxpy solver <https://www.cvxpy.org/tutorial/solvers/index.html#choosing-a-solver>`_
    method         : str = "global"    # "global" fit (one big linear program), "iterative" fit (start from the front) or "individual" fit (each expiry separatedly)
    max_iweight    : float = 100.      # maximum input weight, e.g. cap on 1/spread
    max_dx         : float|None = 0.05 # maximum strike dx for :func:`nbs.iv.sanos.vec_max_dx`
    min_dx         : float = 1E-6      # minimum strike dx for :func:`nbs.iv.sanos.vec_max_dx` 
...
class SmoothCallSurface( object ):
    """
    [SANOS](https://arxiv.org/abs/2601.11209) model fit to multiple expiries.
    
    This implementation uses the same expiries as the market and adds model strikes (e.g. when the strikes in the market are farther apart than ``config.max_dx``).
    The resulting object can be called to compute options at any expiry and strikes; see :func:`nbs.iv.sanos.vec_max_dx`.
    
    Alternatively, model strikes can be specified using ``config.model_strikes``.
    """
    ...
    def __init__( self, market         : Mapping[ExpiryData], *,
        ...
        for j in range(M):
            ...
            def jkernel( strikes ):
                return ( bs.price( strikes[:,None] / jr.xstrikes[None,:], jr.sqrtT*jr.xvols[None,:] ) *  jr.xstrikes[None,:] ) \
            ...
            if j==0 or config.method == "individual":
                ...
                jr.xfloor     = bs.price( jr.xstrikes, jr.sqrtT*config.floor_vol ) if not config.is_linear else np.maximum( 1. - jr.xstrikes, 0. )
                ...
                jxq_m1        = jqs[j-1]
                jxC_m1        = ( bs.price( jr.xstrikes[:,None] / jr_m1.xstrikes[None,:], jr_m1.sqrtT*jr_m1.xvols[None,:] ) *  jr_m1.xstrikes[None,:] ) @ jxq_m1 \
            ...
            if config.bid_ask_mode == "constraint":
                if j>0 and config.method == "iterative":
                    ...
                    floor         = bs.price( jr.strikes, jr.sqrtT*config.floor_vol ) 
    ...
    def __call__( self, strikes : np.ndarray|float, expiry : float):
        ...
        atm_call = bs.price( 1., math.sqrt( atm_var ) )
        ...
        assert lower_atm_call < upper_atm_call, ("ATM prices not increasing...?", lower_atm_call, upper_atm_call )
    ...
    def plot( self, *, 
        ...
        return_figs : bool, default ``False``
            If ``True``, return a list of ``(label, matplotlib.figure.Figure)``
            tuples instead of ``None``.  The caller is responsible for closing
            the returned figures via ``plt.close(fig)``.
        """
        # Normalise dt: accept date, datetime, or string
        if isinstance(dt, date):
            dt = dt.strftime("%Y-%m-%d")
        elif dt is not None:
            dt = str(dt)
        collected_figs: list[tuple[str, object]] | None = [] if return_figs else None
        out_path = None if out_dir is None else SubDir(out_dir)
        for j, jr in enumerate(self.results):
            with figure(f"{f'Smooth' if not self.is_linear else 'Linear'} fitted expiry {jr.label}" +\
                        ("" if dt is None else " @ "+dt)+\
                ...
                iv_xfloor              = bs.implied( jr.xstrikes, jr.xfloor, sqrtT=jr.sqrtT, ret_only_vols=True, on_exceed_bounds="quiet" ) if not jr.xfloor is None and plot_floor_vols else None
                ...
                if out_path is not None:
                    ...
                    fname = out_path.full_file_name( fmt_filename("_".join(name_parts)), ext="pdf")
                    out_path.create_directory()
                    fig.savefig(fname, silent_close=False, bbox_inches='tight')

                # Collect the underlying matplotlib figure before the
                # context manager closes it.
                if collected_figs is not None:
                    # Ensure the figure has been rendered (created) so
                    # that fig._fig is not None.
                    if fig._fig is None:
                        fig.render(draw=False)
                    collected_figs.append((jr.label, fig._fig))
                    # Detach the mpl figure from the DynaFig wrapper so
                    # that fig.close() below does NOT call plt.close()
                    # on it.  The caller (e.g. main_sanos) is now
                    # responsible for closing the collected figures.
                    fig._fig = None

                # When show=False we are running headless (Agg backend,
                # CLI script, etc.).  Explicitly close without rendering
                # to prevent the context-manager __exit__ from calling
                # render(draw=True) which would fail with MODE.HDISPLAY
                # outside an IPython/Jupyter session.
                if not show:
                    fig.close(render=False)

        return collected_figs
