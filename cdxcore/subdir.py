...
class SubDir(object):
    ...
    **Pandas**
    ...
    def cache( self,  version              : str|None = None , *,
        ...
        **Numpy/Pandas/Polars (as inputs)**

        Numpy/Pandas/Polars and similar data should not be hashed for identifying unique call IDs.
        ...
        * ``PYTREE_HDF5``: uses :func:`h5py.File` to write and read dictionaries of data, which can include numpy arrays. Note that HDF5 does not support a wide range of data formats, for example
        ...
        change the format unless it is uniform across a project::
            ...
            def compute_tree( seed ):
                np.random.seed(seed)
                return dict( a=np.random.rand(1000), b=np.random.rand(1000) )
            ...
            ----------
                ...
                cache_generate_only : bool, default ``False``
                    ...
                    * ``True`` if the object did not exist, or had the wrong version, and was therefore re-computed and
                    ...
                    if they already exist. You cannot combine this with ``override_cache_mode="readonly"`` to just test whether the respective correct cache exists.
                    To do so, use ``override_cache_mode="cacheonly"`` instead and catch the :class:`cdxcore.subdir.CacheMustExistError` exception.
                ...
                if r is tag:
                    if cache_mode.must_exist:
                        ...
                        if not rex:
                            ...
                            fnd_version = sub_dir.get_version(filename)
                            reason = f"Version mismatch: expected '{idversion}', found '{fnd_version}'."
                        else:
